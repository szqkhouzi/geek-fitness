...
def search(request):
    if request.method == 'GET':
        ...
        search ={
                'title': {'$in': [re.compile(text)]},
        }

        u = LoveFitness.objects(**search).order_by('_datetime')

        paginator = Paginator(u, 20)
        pageNum = request.GET.get('page', 1)
        ...
        # 页面范围  显示 分页时的 结果， 就是如果有多个分页，不完全显示，就显示4个页面
        #   获取当前页码和 当前页码的前后两页
        page_range = list(range(max(currentPageNum - 2, 1), currentPageNum)) + list(
            range(currentPageNum, min(currentPageNum + 2, paginator.num_pages) + 1))
        ...
        return render(request, "search.html",{'contacts': contacts, 'pageBlogs': pageBlogs,'page_range': page_range,'text':text,'u':u,
                                              'jszs': jszs, 'jslz': jslz, 'jsrd': jsrd, 'jstg': jstg, 'jsyy': jsyy,
