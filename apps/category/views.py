...
def cate(request):
    title = request.GET.get('s_cate','')
    category_list = LoveFitness.objects(s_cate=title)[0]
    s_cate = category_list.s_cate

    types = LoveFitness.objects.filter(s_cate=title).order_by('-datetime')
    print(types.count())
    ...
    jrn = category.s_cate
    return render(request,'cate.html',{'pageNum':str(pageNum),'contacts':contacts,'maxNum':str(maxNum),'types':types,'s_cate':s_cate,'types_count':types_count,'pageBlogs':pageBlogs,'page_range':page_range,
                                       'jszs': jszs, 'jslz': jslz, 'jsrd': jsrd, 'jstg': jstg, 'jsyy': jsyy,
                                       '30days': thirty_days, 'jzsx': jzsx, 'zjzz': zjzz, 'jsdk': jsdk, 'yl': yl,
                                       'xljh': xljh,
